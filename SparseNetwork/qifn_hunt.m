...
% A0Vals = 0.52*sqrt(M); % Up
% A0Vals = 0.5*sqrt(M); % Down
% A0ValsUp = 0.52*sqrt(M);
% A0ValsDown = 0.5*sqrt(M);
A0ValsUp   = 1.600945361816618e+01;
A0ValsDown = 1.600945358046890e+01;
...
for ii = 1:100
  ...
  format long e
  A0 = 0.5*(A0ValsDown(end)+A0ValsUp(end))
...
%   zlabel('$R$','Interpreter','LaTeX');
 ...
 switch flag
   case 'u'
    A0ValsUp = [A0ValsUp; A0];
   case 'd'
    A0ValsDown = [A0ValsDown; A0];
   otherwise
     error('unknown flag');
 end
