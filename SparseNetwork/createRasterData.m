...
%   zlabel('$R$','Interpreter','LaTeX');
  ...
  figure(bd);
  % intSVals(ii) = sum(sMeanHist)*dt/T;
  intSVals(ii) = max(sMeanHist);
  plot(A0Vals(1:ii),intSVals(1:ii),'.-','Color',blue);
  ...
  if saveFiles
    if saveSpikes
      fileName = sprintf('RasterData/rasterData-%06i.mat',ii);
      save(fileName,'A0','tHist','vMeanHist','sMeanHist','rateHist','spikesHist');
    else
      fileName = sprintf('SimulationData/simData-%06i.mat',ii);
      save(fileName,'A0','tHist','vMeanHist','sMeanHist','rateHist');
    end
  end
